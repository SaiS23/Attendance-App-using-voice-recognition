...
public class student_attendance_sheet extends AppCompatActivity {
    ...
    public void display(final String rollno,String classname){
        ...
        dates.clear();
        dates.add("Date          "+"CN  "+"FLAT  "+"MAD  "+"ML  "+ "CG   "+"CGL  "+"CNL  "+"DBMSL");
                ...
                try {
                    for (DataSnapshot dsp : dataSnapshot.getChildren()) {
                        ...
                        if(dsp.child(rollno).hasChild("CN")){
                            ...
                            p1="--";
                        if(dsp.child(rollno).hasChild("FLAT")){
                            p2=dsp.child(rollno).child("FLAT").getValue().toString().substring(0,1);
                        }else
                            p2="--";
                        if(dsp.child(rollno).hasChild("MAD")){
                            p3=dsp.child(rollno).child("MAD").getValue().toString().substring(0,1);
                        }else
                            p3="--";
                        if(dsp.child(rollno).hasChild("ML ")){
                            p4=dsp.child(rollno).child("ML ").getValue().toString().substring(0,1);
                        }else
                            p4="--";
                        if(dsp.child(rollno).hasChild("CG")){
                            p5=dsp.child(rollno).child("CG").getValue().toString().substring(0,1);
                        }else
                            p5="--";
                        if(dsp.child(rollno).hasChild("CGL")){
                            p6=dsp.child(rollno).child("CGL").getValue().toString().substring(0,1);
                        }else
                            p6="--";
                        if(dsp.child(rollno).hasChild("CNL")){
                            ...
                            p7="--";
                        if(dsp.child(rollno).hasChild("MADL")){
                            p8=dsp.child(rollno).child("MADL").getValue().toString().substring(0,1);
                        }else
                            p8="--";
