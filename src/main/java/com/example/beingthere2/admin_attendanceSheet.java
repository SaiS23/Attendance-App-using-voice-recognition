...
public class admin_attendanceSheet extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    public void display_list(final ArrayList userlist){
        ...
        dbAttendance.child(required_date).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                subjectlist.setText("Roll-no  "+"CN  "+"FLAT  "+"MAD  "+"ML   "+ "CG   "+"CGL  "+"CNL  "+"MADL");
                ...
                for (Object rollno : userlist) {
                    ...
                    if(dataSnapshot.child(rollno.toString()).hasChild("CN")){
                        ...
                        p1="--";
                    if(dataSnapshot.child(rollno.toString()).hasChild("FLAT")){
                        p2=dataSnapshot.child(rollno.toString()).child("FLAT").getValue().toString().substring(0,1);
                    }else
                        p2="--";
                    if(dataSnapshot.child("145").hasChild("MAD")){
                        p3=dataSnapshot.child(rollno.toString()).child("MAD").getValue().toString().substring(0,1);
                    }else
                        p3="--";
                    if(dataSnapshot.child(rollno.toString()).hasChild("ML")){
                        p4=dataSnapshot.child(rollno.toString()).child("ML").getValue().toString().substring(0,1);
                    }else
                        p4="--";
                    if(dataSnapshot.child(rollno.toString()).hasChild("CG")){
                        p5=dataSnapshot.child(rollno.toString()).child("CG").getValue().toString().substring(0,1);
                    }else
                        p5="--";
                    if(dataSnapshot.child(rollno.toString()).hasChild("CGL")){
                        p6=dataSnapshot.child(rollno.toString()).child("CGL").getValue().toString().substring(0,1);
                    }else
                        p6="--";
                    if(dataSnapshot.child(rollno.toString()).hasChild("CNL")){
                        ...
                        p7="--";
                    if(dataSnapshot.child(rollno.toString()).hasChild("MADL")){
                        p8=dataSnapshot.child(rollno.toString()).child("MADL").getValue().toString().substring(0,1);
                    }else
                        p8="--";
        ...
        dbAttendance = ref.child("attendance").child(classname).child(required_date);
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                try {
                    for (DataSnapshot dsp : dataSnapshot.getChildren()) {
                        ...
                        if(dsp.hasChild("CN")){
                            p1=dsp.child("CN").getValue().toString().substring(0,1);
                        }else
                            p1="--";
                        if(dsp.hasChild("FLAT")){
                            p2=dsp.child("FLAT").getValue().toString().substring(0,1);
                        }else
                            p2="--";
                        if(dsp.hasChild("MAD")){
                            p3=dsp.child("MAD").getValue().toString().substring(0,1);
                        }else
                            p3="--";
                        if(dsp.hasChild("ML")){
                            p4=dsp.child("ML").getValue().toString().substring(0,1);
                        }else
                            p4="--";
                        if(dsp.hasChild("CG")){
                            p5=dsp.child("CG").getValue().toString().substring(0,1);
                        }else
                            p5="--";
                        if(dsp.hasChild("CGL")){
                            p6=dsp.child("CGL").getValue().toString().substring(0,1);
                        }else
                            p6="--";
                        if(dsp.hasChild("CNL")){
                            p7=dsp.child("CNL").getValue().toString().substring(0,1);
                        }else
                            p7="--";
                        if(dsp.hasChild("MADL")){
                            p8=dsp.child("MADL").getValue().toString().substring(0,1);
                        }else
                            p8="--";
                        Studentlist.add( rollno1+"    "+p1+"     "+p2+"     "+p3+"     "+p4+"      "+p5+"       "+p6+"      "+p7+"      "+p8);

                    }
                    list2(Studentlist);
                }catch (Exception e){
                    Toast.makeText(admin_attendanceSheet.this,"Something went wrong...Please try again", Toast.LENGTH_SHORT).show();
                }
            }
